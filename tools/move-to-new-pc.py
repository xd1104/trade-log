...
`AUTO_ORDERS_ON` —— **那個檔存在就代表「每個交易日 09:03:30 程式會自己送單」**（2026-09-09 起）。
同一個理由，而且更嚴重：`REAL_ORDERS_ON` 至少要他人在場按鈕才送，
這個是**他不在也會送**。新電腦確認連得上永豐之後，用面板上的「用 A／B 開始」重新打開。
⚠️ `autofire/`（自動下單的帳本）**照樣打包** —— `auto_fire.arm()` 只看開關檔、不看帳本，
帶過去不會讓新電腦自己開始送單；反而 `_has(今天)` 讀得到帳本，
同一天搬家不會在新電腦上再送第二張。

⛔ **兩台電腦不可以同時開著面板**：
  - 兩台都開著自動下單 ⇒ 09:03:30 各送一張 ⇒ **兩口真單**
  - 永豐的登入名額有限，新電腦登入會把舊電腦踢下線 ⇒ 舊電腦上如果還有部位，
    **它的停損就沒了**（停損活在面板迴圈裡，不在券商那邊）
...
GROUPS = [
    ("你的紀錄（repo 裡沒有，這是唯一一份）", [
        ...
        (SJ / "autofire", "【自動下單】的帳本（哪天送了什麼、怎麼收的）", False),
        (SJ / "autotest", "【自動下單（模擬）】四種做法的模擬紀錄", False),
        (SJ / "tick_logs", "每天 08:45~09:30 錄下來的逐筆報價（【細節】分頁的來源）", False),
...
⛔ 舊電腦：搬之前先做這兩件事
-------------------------------------------------
  1) 面板【自動下單】那一頁按「關閉自動下單」
  2) 把面板整個關掉（連看門狗 start-panel.bat 那個黑視窗一起關）

**兩台電腦不可以同時開著面板**：
  - 兩台都開著自動下單 ⇒ 09:03:30 各送一張 ⇒ 兩口真單
  - 永豐的登入名額有限，新電腦一登入就會把舊電腦踢下線 ⇒
    舊電腦上如果還有部位，它的停損就沒了
...
自動下單也一樣沒有帶過去。確認上面都正常之後，到面板【自動下單】那一頁
按「用 A 開始」或「用 B 開始」→ 確認條 →「確定，打開」。
（你過去的自動下單紀錄有帶過去，那一頁看得到。）
...
def main():
    ...
    # 兩個開關檔一定要講出來，不然他會以為打包 = 全部都在
    real_on = (SJ / "REAL_ORDERS_ON").exists()
    auto_on = (SJ / "AUTO_ORDERS_ON").exists()
    if real_on or auto_on:
        print("【刻意沒有打包】")
    if real_on:
    ...
    if auto_on:
        print("  跳過  tools/shioaji/AUTO_ORDERS_ON")
        print("        那個檔存在 ＝ 每個交易日 09:03:30 程式會自己送單（你不在也會送）。")
        print("        新電腦確認連得上永豐之後，用面板上的「用 A／B 開始」重新打開。")
    if real_on or auto_on:
        print()
    if auto_on:
        # ⛔ 這台現在還開著自動下單 —— 搬家最危險的就是兩台同時在跑
        print("⛔ 這台電腦的自動下單現在是【開著】的。搬之前一定要：")
        print("   1) 面板【自動下單】那一頁按「關閉自動下單」")
        print("   2) 把這台的面板整個關掉")
        print("   兩台同時開著 ⇒ 09:03:30 各送一張 ⇒ 兩口真單；")
        print("   而且新電腦一登入永豐，就會把這台踢下線 ⇒ 這台如果還有部位，停損就沒了。\n")
