...
M = [
    ...
    ("N6c orb 中位數改成平均", S, "    return float(np.median(np.asarray(box_vals, dtype=float)[-ORB_HIST_N:]))", "    return float(np.mean(np.asarray(box_vals, dtype=float)[-ORB_HIST_N:]))"),
    ("N7 orb 歷史不夠當成定論（會寫檔）", S, '        return _pending("few_box_hist", "箱子寬度歷史不夠（這天以前只有 %d 天，要 %d 天）"\n                        % (len(bh), ORB_HIST_N))', '        return _none_row("orb", day, "few_box_hist", "箱子寬度歷史不夠")'),
    ("N7c orb 歷史天數不夠也給中位數", S, "    if box_vals is None or len(box_vals) < ORB_HIST_N:\n        return None", "    if box_vals is None or len(box_vals) < 1:\n        return None"),
    ...
    ("U3 不用共用的候選、自己重算一次", S, "    p = pack if pack is not None else day_pack(day, D, hist_rows, box_vals, c)\n    cands, stop = union_cands(day, D, p)", '    p = day_pack(day, D, hist_rows, (pack["box"] if pack is not None else box_vals), c)\n    cands, stop = union_cands(day, D, p)'),
    ("U4 快的日子也算回馬槍候選", S, '    if not ctx["fast"]:                         ', "    if True:                         "),
    ("U5 開箱候選不看箱子濾網", S, '    if o is not None and o["i"] is not None and o["box_pct"] >= med:', '    if o is not None and o["i"] is not None:'),
    ("U5b 開箱候選要「大於」中位數才算", S, '    if o is not None and o["i"] is not None and o["box_pct"] >= med:', '    if o is not None and o["i"] is not None and o["box_pct"] > med:'),
    ...
    ("U8 箱子歷史不夠當成定論（會寫檔）", S, '        return None, {"pending": True, "why": "few_box_hist",\n                      "msg": ', '        return None, {"why": "few_box_hist",\n                      "reason": '),
