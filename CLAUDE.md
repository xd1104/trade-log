...
- **模擬帳戶已經跑通整條路**（2026-09-01，`test_broker_sim.py`）：
  進場成交 → 停利掛上（Submitted）→ 對帳看得到部位 → 撤停利 → 市價平倉 → 空手。
  兩輪都正常。**這是唯一驗證過「真的送出去」那一段的方式**，改過 `broker.py` 的下單
  流程就要重跑。⚠️ 那支會真的呼叫 `place_order`，只是連的是模擬帳戶。
